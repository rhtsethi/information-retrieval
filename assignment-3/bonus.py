...
def calc_tf_idf_score(terms):
    global tf_idf_document_term
    for term in terms:
        posting_list=inverted_index[term]
        df=posting_list[0][0]
        for i in range(1,len(posting_list)):
            if(posting_list[i][0] not in tf_idf_document_term):
                tf_idf_document_term[posting_list[i][0]]={}        
            tf_idf_document_term[posting_list[i][0]][term]=((1+ math.log10(posting_list[i][1]))*(math.log10((N_docs*1.0)/df)))
 
def calc_new_vocabulary():
    global terms
    global k_docs
    terms=set()
    count=0
#    for key_,value in tf_idf_document_term:
#        tf_idf_document_term[key_]=sorted(tf_idf_document_term[key_], key=tf_idf_document_term[key_].get,reverse=True)
        #tf_idf_document_term[key_].sort(key=lambda x:x[1],reverse=True)
    for key_,value in tf_idf_document_term.iteritems():
        for term in sorted(tf_idf_document_term[key_], key=tf_idf_document_term[key_].get,reverse=True):
            if count>=k_docs:
                break
